...
function activate(context) {
    ...
    context.subscriptions.push(
        vscode.workspace.registerTextDocumentContentProvider('todotree-export', {
    ...
    function updateInformation() {
        ...
        if (statusBar === STATUS_BAR_CURRENT_FILE) {
            ...
            total = Object.values(counts).reduce(function (a, b) {
                return a + b;
            }, 0);
        }
        ...
        if (statusBar === STATUS_BAR_TOTAL) {
            ...
            if (statusBar === STATUS_BAR_TOP_THREE) {
                sortedTags.sort(function (a, b) {
                    return counts[a] < counts[b] ? 1 : counts[b] < counts[a] ? -1 : a > b ? 1 : -1;
                });
                ...
                vscode.window.showInformationMessage(message, ...buttons).then(function (button) {
                    ...
                    if (button === undefined) {
                        ...
                        vscode.env.openExternal(
                            vscode.Uri.parse('https://github.com/Gruntfuggly/todo-tree#markdown-support')
                        );
    ...
    function register() {
        function migrateSettings() {
            ...
            if (context.globalState.get('migratedVersion', 0) < 210) {
                ...
                if (
                    validValues.indexOf(vscode.workspace.getConfiguration('todo-tree.general').revealBehaviour) === -1
                ) {
                    vscode.window
                        .showInformationMessage(
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.exportTree', function () {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.showOnlyThisFolder', function (node) {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.showOnlyThisFolderAndSubfolders', function (node) {
                var rootNode = tree.locateWorkspaceNode(node.fsPath);
                var includeGlobs = [utils.createFolderGlob(node.fsPath, rootNode.fsPath, '/**/*')];
                context.workspaceState.update('includeGlobs', includeGlobs);
                rebuild();
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.switchScope', function () {
                    ...
                    vscode.window.showQuickPick(items, options).then(function (scope) {
                        if (scope) {
                            ...
                            context.workspaceState.update(
                                'includeGlobs',
                                utils.toGlobArray(currentConfig.includeGlobs)
                            );
                            context.workspaceState.update(
                                'excludeGlobs',
                                utils.toGlobArray(currentConfig.excludeGlobs)
                            );
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.excludeThisFolder', function (node) {
                var rootNode = tree.locateWorkspaceNode(node.fsPath);
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.excludeThisFile', function (node) {
                var excludeGlobs = context.workspaceState.get('excludeGlobs') || [];
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.removeFilter', function () {
                ...
                includeGlobs.forEach(function (includeGlob) {
                    if (includeGlob.endsWith('/**/*')) {
                        choices['Include Folder and Subfolders: ' + includeGlob.slice(0, -5)] = {
                            include: includeGlob,
                        };
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.resetAllFilters', function () {
                context.workspaceState.update('includeGlobs', []);
                context.workspaceState.update('excludeGlobs', []);
                rebuild();
                dumpFolderFilter();
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.goToNext', function () {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.goToPrevious', function () {
                var editor = vscode.window.activeTextEditor;
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.revealInFile', function (uri, selection) {
                function flashLine() {
                    var editor = vscode.window.activeTextEditor;
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.scanWorkspaceAndOpenFiles', scanWorkspaceAndOpenFiles)
        );
        context.subscriptions.push(vscode.commands.registerCommand('todo-tree.scanOpenFilesOnly', scanOpenFilesOnly));
        context.subscriptions.push(
            vscode.commands.registerCommand('todo-tree.scanCurrentFileOnly', scanCurrentFileOnly)
        );
        ...
        context.subscriptions.push(
            vscode.window.onDidChangeActiveTextEditor(function (e) {
                if (e && e.document) {
                    ...
                    if (
                        vscode.workspace.getConfiguration('todo-tree.tree').autoRefresh === true &&
                        vscode.workspace.getConfiguration('todo-tree.tree').trackFile === true
                    ) {
