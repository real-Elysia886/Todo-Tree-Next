...
var utils = require('../src/utils.ts');
var attributes = require('../src/attributes.ts');
var stubs = require('./stubs.js');
var searchResults = require('../src/searchResults.ts');
...
QUnit.test(
    "utils.getRegexSource returns the regex source without expanded tags if they aren't present",
    function (assert) {
        var testConfig = stubs.getTestConfig();
...
QUnit.test(
    'utils.getRegexSource sorts the tags in reverse order to allow more specific tags to be found first',
    function (assert) {
        var testConfig = stubs.getTestConfig();
...
QUnit.test('utils.getRegexSource returns the regex source and escapes other regex characters', function (assert) {
    var testConfig = stubs.getTestConfig();
    testConfig.tagList = ['A|B', 'A{B', 'A^B', 'A[B', 'A?B', 'A.B', 'A+B', 'A*B', 'A)B', 'A(B', 'A$B'];
...
QUnit.test('utils.formatLabel replaces column number placeholder', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${column} content', { column: 78 }, unexpectedPlaceholders),
        'Label 78 content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces before text placeholder', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${before} content', { before: 'text before tag' }, unexpectedPlaceholders),
        'Label text before tag content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces tag placeholders', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${tag} content', { actualTag: 'Todo' }, unexpectedPlaceholders),
        'Label Todo content'
    );
    assert.equal(
        utils.formatLabel('Label ${tag:uppercase} content', { actualTag: 'todo' }, unexpectedPlaceholders),
        'Label TODO content'
    );
    assert.equal(
        utils.formatLabel('Label ${tag:lowercase} content', { actualTag: 'TODO' }, unexpectedPlaceholders),
        'Label todo content'
    );
    assert.equal(
        utils.formatLabel('Label ${tag:capitalize} content', { actualTag: 'todo' }, unexpectedPlaceholders),
        'Label Todo content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces sub tag placeholders', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${subTag} content', { subTag: 'name@mail.com' }, unexpectedPlaceholders),
        'Label name@mail.com content'
    );
    assert.equal(
        utils.formatLabel('Label ${subtag} content', { subTag: 'Name@mail.com' }, unexpectedPlaceholders),
        'Label Name@mail.com content'
    ...
    assert.equal(
        utils.formatLabel('Label ${subtag:uppercase} content', { subTag: 'example' }, unexpectedPlaceholders),
        'Label EXAMPLE content'
    );
    assert.equal(
        utils.formatLabel('Label ${subtag:lowercase} content', { subTag: 'EXAMPLE' }, unexpectedPlaceholders),
        'Label example content'
    );
    assert.equal(
        utils.formatLabel('Label ${subtag:capitalize} content', { subTag: 'example' }, unexpectedPlaceholders),
        'Label Example content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces after text placeholder', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${after} content', { after: 'text after tag' }, unexpectedPlaceholders),
        'Label text after tag content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces filename placeholder', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${filename} content', { fsPath: '/path/to/filename.txt' }, unexpectedPlaceholders),
        'Label filename.txt content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.formatLabel replaces filepath placeholder', function (assert) {
    var unexpectedPlaceholders = [];
    assert.equal(
        utils.formatLabel('Label ${filepath} content', { fsPath: '/path/to/filename.txt' }, unexpectedPlaceholders),
        'Label /path/to/filename.txt content'
    );
    assert.equal(unexpectedPlaceholders.length, 0);
});
...
QUnit.test('utils.createFolderGlob creates expected globs', function (assert) {
    if (process.platform === 'win32') {
        ...
        assert.equal(
            utils.createFolderGlob(
                '/Users/name/workspace/project/folder/subfolder',
                '/Users/name/workspace/project',
                '/**/*'
            ),
            '/Users/name/workspace/project/folder/subfolder/**/*'
        );
...
QUnit.test('utils.formatExportPath inserts date and time fields', function (assert) {
    ...
    assert.equal(
        utils.formatExportPath('todo-tree-%Y%m%d-%H%M-export', new Date(1307472705067)),
        'todo-tree-' + expectedDateTime + '-export'
    );
});
...
QUnit.test('utils.formatExportPath expands tilde', function (assert) {
    ...
    assert.equal(
        utils.formatExportPath('~/todo-tree-%A', new Date(1307472705067)),
        path.join(homeFolder, 'todo-tree-' + expectedDateTime)
    );
});
...
QUnit.test('attributes.getForeground uses colour scheme', function (assert) {
    ...
    testConfig.tagList = ['FIXME', 'TODO', 'BUG'];
...
QUnit.test('filterQuery matches plain terms', function (assert) {
    var matcher = filterQuery.createMatcher('auth', false);
    assert.ok(matcher({ label: 'TODO refactor auth flow', fsPath: 'src/login.js', actualTag: 'TODO' }));
    assert.notOk(matcher({ label: 'TODO refactor payments', fsPath: 'src/pay.js', actualTag: 'TODO' }));
});

QUnit.test('filterQuery supports field queries', function (assert) {
    var matcher = filterQuery.createMatcher('tag:TODO path:src text:auth priority:P0', false);

    assert.ok(
        matcher({
            label: 'TODO auth cleanup',
            after: 'auth cleanup',
            fsPath: 'src/login.js',
            actualTag: 'TODO',
            priority: 'P0',
...
QUnit.test('filterQuery supports markdown status', function (assert) {
    var openMatcher = filterQuery.createMatcher('status:open', false);
    var doneMatcher = filterQuery.createMatcher('status:done', false);

    assert.ok(openMatcher({ label: 'write docs', actualTag: '[ ]' }));
    assert.notOk(doneMatcher({ label: 'write docs', actualTag: '[ ]' }));
    assert.ok(doneMatcher({ label: 'publish docs', actualTag: '[x]' }));
});

QUnit.test('gitFiles parses porcelain status', function (assert) {
    var entries = gitFiles.parseStatus(' M src/a.js\0A  src/b.js\0?? src/c.js\0');

    assert.equal(entries.length, 3);
    assert.equal(entries[0].indexStatus, ' ');
    assert.equal(entries[0].worktreeStatus, 'M');
    assert.equal(entries[0].file, 'src/a.js');
    assert.equal(entries[1].indexStatus, 'A');
    assert.equal(entries[2].indexStatus, '?');
});

QUnit.test('gitFiles parses porcelain rename target', function (assert) {
    var entries = gitFiles.parseStatus('R  old.js\0new.js\0');
