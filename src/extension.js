...
function activate( context )
{
    ...
    function search( options )
    {
        ...
        {
            return scannerClient.scanWorkspace( context, options.filename, options ).then( function( matches )
            {
    ...
    {
        return config.isValidScheme( document.uri ) &&
            isIncluded( document.uri ) === true &&
            ( config.scanMode() !== SCAN_MODE_CURRENT_FILE ||
                ( vscode.window.activeTextEditor && document.fileName === vscode.window.activeTextEditor.document.fileName ) );
    }
    ...
    function scanGitFiles( staged )
    {
        gitScanner.scanGitFiles( {
            ...
            debug: debug,
            setInterrupted: function( value ) { interrupted = value; }
        }, staged );
    }
    ...
    {
        if( shouldScanDocument( document ) && scannerClient.enabled( context, getOptions( getRootForFile( document.fileName ) ) ) )
        {
    ...
    function register()
    {
        ...
        context.subscriptions.push( vscode.commands.registerCommand( 'todo-tree.openDashboard', function()
        {
            dashboard.show( context, provider, {
                ...
                scanChangedFiles: function() { scanGitFiles( false ); },
                scanStagedFiles: function() { scanGitFiles( true ); }
            } );
        } ) );
        ...
        commands.registerMany( context, [
            ...
            { command: 'todo-tree.scanChangedFilesOnly', handler: function() { scanGitFiles( false ); } },
            { command: 'todo-tree.scanStagedFilesOnly', handler: function() { scanGitFiles( true ); } },
        ...
        fileWatcher.register( context, {
            ...
            getSelectedDocument: function() { return selectedDocument; },
            setSelectedDocument: function( value ) { selectedDocument = value; },
